...
import static ca.pirurvik.iutools.corpus.CorpusCompilationProgress.Phase;
...
public class CorpusCompiler {
	...
	public void compileWordFrequencies(String _corpusName) throws CorpusCompilerException {
		progress.setCurrentPhase(Phase.COMPUTE_WORD_FREQUENCIES);
		this.corpusName = _corpusName;
	...
	private boolean performNextStep() throws CorpusCompilerException {
		...
		if (progress.currentPhase == null) {
			progress.currentPhase = Phase.COMPUTE_WORD_FREQUENCIES;
		}
		toConsole(true,
			"\n=============================\nStarting phase: "+progress.currentPhase);
		Phase oldPhase = progress.currentPhase;
		if (progress.currentPhase == Phase.COMPUTE_WORD_FREQUENCIES) {
			/**
			 ...
			 */
			phaseCompileFreqsCorpusFiles();
		} else if (progress.currentPhase == Phase.GENERATE_CORP_FILE_NO_DECOMPS) {
			/**
		...
		} else if (progress.currentPhase == Phase.LOAD_CORPUS_NODECOMPS) {
			 /**
		...
		} else if (progress.currentPhase == Phase.CHECK_DECOMPS_FILE) {
			/**
		...
		} else if (progress.currentPhase == Phase.GENERATE_CORP_FILE_WITH_DECOMPS) {
			/**
		...
		} else if (progress.currentPhase == Phase.LOAD_FINALIZED_CORPUS) {
		...
		} else if (progress.currentPhase == Phase.REFORMAT_FOR_VERSION_TRACKING) {
			/**
	...
	public  void phaseCompileFreqsCorpusFiles() throws CorpusCompilerException {
		...
		progress.currentPhase = Phase.GENERATE_CORP_FILE_NO_DECOMPS;
	}
	...
	private void phaseGenerateCorpFileNoDecomps() throws CorpusCompilerException {
		...
		progress.currentPhase = Phase.LOAD_CORPUS_NODECOMPS;
	}
	...
	private void phaseLoadCorpusNoDecomps() throws CorpusCompilerException {
		...
		progress.currentPhase = Phase.CHECK_DECOMPS_FILE;
	}
	...
	private boolean phaseCheckDecompsFile() throws CorpusCompilerException {
		...
		"Decompositions file does not exist:\n   "+decompsFile+"\n"+
			...
			progress.currentPhase = Phase.GENERATE_CORP_FILE_WITH_DECOMPS;
		}
		return stop;
	}
	...
	private void phaseGenerateCorpFileWithDecomps() throws CorpusCompilerException {
		...
		progress.currentPhase = Phase.LOAD_FINALIZED_CORPUS;
	...
	private void phaseLoadFinalizedCorpusNoDecomps() throws CorpusCompilerException {
		...
		progress.currentPhase = Phase.REFORMAT_FOR_VERSION_TRACKING;
	}
	...
	private void phaseReformatForVersionTracking() throws CorpusCompilerException {
		...
		progress.currentPhase = Phase.DONE;
	}
...
//				}
	...
	private void saveProgress() throws CorpusCompilerException {
		boolean done = progress.currentPhase == Phase.DONE;
		if (done || progress.currentPhase == Phase.COMPUTE_WORD_FREQUENCIES) {
			saveWordFrequencies();
		}
		writeProgressFile();
		lastSaveMSecs = StopWatch.nowMSecs();
