...
//
// Description: Pour le d�bogage de m�thodes de fa�on syst�matique,
//              coh�rente et globale.
//
// -----------------------------------------------------------------------
...
public class Debugging {
	...
	// Cr�ation d'une fen�tre de dialogue dans laquelle les fonctions
	// d�bogagles sont affich�es avec un bouton radio � cocher pour
	// activer le d�bogage des fonctions.
	static {
...
//		}
	...
	private class FenetreDebogage {
		...
		public FenetreDebogage() {
			...
			// 2 parties: 1) les boutons de la fen�tre (cacher, ...)
			//            2) les champs
			Box boite1 = Box.createHorizontalBox();
			JButton bq = new JButton("R�duire");
			...
			// b1: D�claration de nouveau point de d�bogage
			// b2: Liste des points de d�bogage
			b1 = Box.createVerticalBox();
			b1a = Box.createHorizontalBox();
			b1b = Box.createHorizontalBox();
			b2 = Box.createVerticalBox();
			ct1 = new JTextField("Nouveau point de d�bogage:");
			...
			b1.add(b1b);
			boite2.add(b1);
			// Cr�er les checkbox (dans b2)
			creerCheckBox();
			sp = new JScrollPane(b2);
			boite2.add(sp);

			fr = new JFrame("D�bogage");
...
//			System.out.println("pas de fichier 'ressources\\debogage.txt'");
	...
	static void ecrireDebogage() {
		...
		//		System.out.println("deb: number de cl�s: "+deb.size());
		File f = new File(fileName);
		if (!f.exists())
			return;
		try {
